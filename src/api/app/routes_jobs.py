...
@router.patch("/{job_id}", response_model=JobCreateResponse)
def update_job(req: JobCreateRequest, job_id: str) -> JobCreateResponse:
    container = get_cosmos_container()
    try:
        item = container.read_item(item=job_id, partition_key="JOB")
        item["categorie"] = req.categorie
        item["status"] = req.status
        item["updateAt"] = job_to_entity(JobCreateRequest(fileName=item["fileName"], contentType=item["contentType"]))["updateAt"]
